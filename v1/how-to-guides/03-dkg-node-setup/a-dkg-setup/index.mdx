...
5. Set up the node as a system service. Run the following commands to create the service configuration file:
...
6. Double check that the config has been written to /etc/systemd/system/standalone.service correctly. If so, enable the service so it runs on startup, and then try to start it now:
