...
// - Finalize on Egg Network name  
// - Update page to properly refer to Webb vs. Egg Network 

<!-- END -->

The Kusama Network allows Parachains to source tokens for their Parachain bids in a decentralized crowdloan. 
Webb will be conducting a crowdloan campaign to secure a Parachain slot on Kusama for the Egg network.
...
- The Eggnet Crowdloan is a community-backed launch for individuals looking to contribute to the growth of the Webb ecosystem and applications. 
...
- Reward distribution is set at 25 EGG for every 1 KSM contributed. 
- EGG tokens can be used for voting on governance proposals. 
...
lease. EGG token (the native token of Egg Network) is distributed based on the amount a user locks and when 
the user locks. 
