...
The relayer allows for submitting proofs for privacy-preserving transactions against the Mixer, Anchor and VAnchor protocols. The users generate zero-knowledge proof data, format a proper payload, and submit it to a compatible relayer for submission.
...
The relayer is tasked with relaying signed data payloads from the DKG’s activities and plays an important role as it pertains to the Anchor Protocol. The relayer is responsible for submitting the unsigned and signed anchor update proposals to and from the DKG before and after signing occurs.
...
1. he relayers are listening to and proposing updates.
