...
A blockchain oracle is a system that listens to an external data source and posts this data back on-chain to arbitrarily many blockchains. Examples of data sources include:

   * Price feeds of USD/ETH pair (Chainlink)
   * Weather feeds in Europe
   * Merkle tree roots on different anchors (Webb)

In the Webb Protocol, relayers fulfill the role of an oracle where the external data sources that they listen to are the state of the anchors for a bridge. The flow for a relayer is the following:

1. Listen to all the anchors of a bridge.
2. When a new deposit / insertion occurs in one of the merkle trees, say `A`, the relayer will construct
...
2. The DKG is signing these updates using a [threshold-signature scheme](https://en.wikipedia.org/wiki/Threshold_cryptosystem).
    1. We say its a threshold signature because it requires a threshold of participants *t-out-of-n* to generate a full signature.
