...
The Tangle Network allows transactions to be validated through a web of approvals by participants in the network. When a new transaction is created, the issuer must approve two previous transactions. This forms a directed acyclic graph (DAG) of approvals that underlie consensus.

In the Anchor System, the Validation Protocol can leverage the Tangle Network to validate anchor updates as follows:

A proposed anchor update is created as a special "anchor update" transaction on the Tangle. This contains information about the change.
The issuer references two previous transactions on the Tangle to approve in order to issue the anchor update transaction.
Nodes on the Tangle verify the anchor update transaction is valid based on Tangle rules and does not conflict with the ledger state.
Once the transaction accumulates sufficient approvals through the Tangle consensus process, it is confirmed.
Anchor contracts treat confirmed Tangle transactions as valid state updates. The anchor processes the update request on-chain.
The anchor's state is updated if the Tangle transaction is sufficiently approved according to parameters set for "anchoring" Tangle transactions.
This allows the Anchor System to benefit from the decentralization and security of the Tangle Network for update validation. Tangle participants help validate anchor changes.

The flexibility to swap in different validation networks is a key benefit of the Anchor System. Different use cases may call for different tradeoffs between decentralization, scalability, and expressiveness.
...
## Liveness and the External Governance System
