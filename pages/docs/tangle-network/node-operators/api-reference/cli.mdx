...
<Tabs items={["docker", "systemd"]} storageKey="selected-binary">
  ...
  <Tab>
    ...
    ./tangle-parachain --help
     
    # Or if you compiled the binary
    ./target/release/tangle-parachain --help
    ```
...
```sh
tangle-parachain --blocks-pruning 120
```
...
```sh
tangle-parachain --bootnodes /ip4/127.0.0.1/tcp/30333/p2p/12D3KooWAWueKNxuNwMbAtss3nDTQhMg4gG3XQBnWdQdu2DuEsZS
```
...
```sh
tangle-parachain --chain arana-local
```
...
```sh
tangle-parachain --base-path /data
```
...
```sh
tangle-parachain --detailed-log-output
```
...
```sh
tangle-parachain --execution wasm
```
...
```sh
tangle-parachain --keystore-path /tmp/chain/data/
```
...
```sh
tangle-parachain --keystore-uri foo://example.com:8042/over/
```
...
```sh
tangle-parachain --node-key b6806626f5e4490c27a4ccffed4fed513539b6a455b14b32f58878cf7c5c4e68
```
...
```sh
tangle-parachain --node-key-file ./node-keys-file/
```
...
```sh
tangle-parachain --prometheus-external
```
...
```sh
tangle-parachain --prometheus-port 9090
```
...
```sh
tangle-parachain --rpc-port 9933
```
...
```sh
tangle-parachain --state-pruning 128
```
...
```sh
tangle-parachain --telemetry-url 'wss://foo/bar 0'
```
...
```sh
tangle-parachain --wasm-execution compiled
```
