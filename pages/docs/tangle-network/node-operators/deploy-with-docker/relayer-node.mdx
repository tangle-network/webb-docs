...
Now that we have generated the keys, we can start the Tangle Collator and Relayer. We will use the `docker-compose` file provided
in the [Tangle repo](https://github.com/webb-tools/tangle/tree/main/docker/tangle-parachain-with-relayer).
...
The `docker-compose` file will spin up a container running Tangle Collator node and another running a Webb Relayer.
