...
import RelayerConfigurationOptions from "../../../../components/RelayerConfigurationOptions";
...
to the Relayer Overview page [here](../overview).
...
configuration and available endpoints please refer to the data-querying page [here](../data-querying).
...
on Goerli Testnet. For an exhasutive list of configuration please refer to the <RelayerConfigurationOptions branch={"main"}/> in the `relayer` repo. Let's add the following to the file:
...
```bash
# There is no need to change anything in this file, you can
# just copy it as is.
{
	# Remove the below line to disable debug logging, could be helpful but noisy.
	#debug
...
```bash
{
  ...
  substrate: {},
  experimental: {
    smartAnchorUpdates: false,
    smartAnchorUpdatesRetries: 0,
  },
  features: {
    dataQuery: true,
    governanceRelay: false,
