...
`Anchor`s are augmented nodes that possess graph-like data structures and functionality; that is, they can be connected and have edges. Users interact with Anchors through a deposit/transfer/withdraw API. To deposit into a Webb Anchor, a user generates a hash commitment and submits this for insertion in the Anchor's merkle tree. The commitment contains the chainID of the chain they wish to withdraw on as well as some secret data.
